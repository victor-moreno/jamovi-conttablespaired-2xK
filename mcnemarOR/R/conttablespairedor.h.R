...
contTablesPairedOROptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "contTablesPairedOROptions",
...
contTablesPairedORResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "contTablesPairedORResults",
...
contTablesPairedORBase <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "contTablesPairedORBase",
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(1,0,0),
                options = options,
                results = contTablesPairedORResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
#'     check.names=FALSE)
#'
#' mcnemarOR::contTablesPairedOR(formula = Counts ~ `1st survey`:`2nd survey`, data = dat)
#'
#' #
#' #  PAIRED SAMPLES CONTINGENCY TABLES
#' #
...
#'
#' mcnemarOR::contTablesPairedOR(formula = ~ `1st survey`:`2nd survey`, data = dat)
#'
...
contTablesPairedOR <- function(
    ...
    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("contTablesPairedOR requires jmvcore to be installed (restart may be required)")
    ...
    options <- contTablesPairedOROptions$new(
        rows = rows,
        cols = cols,
        counts = counts,
        chiSq = chiSq,
    ...
    analysis <- contTablesPairedORClass$new(
        options = options,
        data = data)
